...
Repository-wide coding standards are documented in the [Copilot instructions](.github/copilot-instructions.md) and focused instruction files in [`.github/instructions`](.github/instructions). In particular:

- Comments explain intent and non-obvious decisions, not code that is already self-explanatory.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc with documented parameters and return values.
- Reusable Astro components document their public `Props` interfaces.
- TypeScript uses single quotes, semicolons, trailing commas in multiline structures, two-space indentation, and explicit function types; ESLint enforces explicit public module-boundary types for data-layer exports.
